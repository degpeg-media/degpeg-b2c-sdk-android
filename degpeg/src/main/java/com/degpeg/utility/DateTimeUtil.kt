...
internal object DateTimeUtil {
    ...
    val UTC_FORMAT = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'", Locale.ENGLISH)
    val LOCAL_FORMAT = SimpleDateFormat("MMM dd, EEEE @ h:mm a", Locale.ENGLISH)
    ...
    fun isTomorrow(date: String, sourceFormat: SimpleDateFormat = UTC_FORMAT): Boolean {
        val localDateTime = date.utcToLocal(sourceFormat)
    ...
    fun String?.utcToLocal(format: SimpleDateFormat = LOCAL_FORMAT): String {
        if (this.isNullOrEmpty()) return ""
        try {
            UTC_FORMAT.timeZone = TimeZone.getTimeZone("UTC");
            val utcDate = UTC_FORMAT.parse(this) ?: return ""
    ...
    fun currentUTCTime(format: SimpleDateFormat = UTC_FORMAT): String {
        return UTC_FORMAT.format(Calendar.getInstance(TimeZone.getTimeZone("UTC")).time)
    }
